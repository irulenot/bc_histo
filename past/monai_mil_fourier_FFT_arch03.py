...
def main_worker(gpu, args):
    ...
    class MyModel(nn.Module):
        def __init__(self):
            ...
            self.x = 256
            self.conv2d_x1_1 = nn.Conv2d(in_channels=1024, out_channels=self.x, kernel_size=1)
            self.conv2d_x2_1 = nn.Conv2d(in_channels=1024, out_channels=self.x, kernel_size=1)
            self.bn_x1_1 = nn.BatchNorm2d(self.x)
            self.bn_x2_1 = nn.BatchNorm2d(self.x)
            self.prelux_x1_1 = nn.PReLU()
            self.prelux_x2_1 = nn.PReLU()

            self.conv2d_x1_2 = nn.Conv2d(in_channels=1024, out_channels=self.x, kernel_size=1)
            self.conv2d_x2_2 = nn.Conv2d(in_channels=1024, out_channels=self.x, kernel_size=1)
            self.bn_x1_2 = nn.BatchNorm2d(self.x)
            self.bn_x2_2 = nn.BatchNorm2d(self.x)
            self.prelux_x1_2 = nn.PReLU()
            self.prelux_x2_2 = nn.PReLU()

            self.conv2d_x1_3 = nn.Conv2d(in_channels=3, out_channels=1, kernel_size=3, padding=1)
            self.conv2d_x2_3 = nn.Conv2d(in_channels=3, out_channels=1, kernel_size=3, padding=1)
            self.bn_x1_3 = nn.BatchNorm2d(1)
            self.bn_x2_3 = nn.BatchNorm2d(1)
            self.prelux_x1_3 = nn.PReLU()
            self.prelux_x2_3 = nn.PReLU()

            self.conv2d = nn.Conv2d(in_channels=2, out_channels=self.x, kernel_size=1)
            self.bn0 = nn.BatchNorm2d(self.x)
            self.prelux0 = nn.PReLU()

            self.conv3d_1 = nn.Conv3d(in_channels=1, out_channels=4, kernel_size=3, stride=2, padding=1)
            self.conv3d_2 = nn.Conv3d(in_channels=4, out_channels=16, kernel_size=3, stride=2, padding=1)
            self.conv3d_3 = nn.Conv3d(in_channels=16, out_channels=64, kernel_size=3, stride=2, padding=1)
            self.bn1 = nn.BatchNorm3d(4)
            self.bn2 = nn.BatchNorm3d(16)
            self.bn3 = nn.BatchNorm3d(64)
            self.prelux1 = nn.PReLU()
            self.prelux2 = nn.PReLU()
            self.prelux3 = nn.PReLU()

            self.conv3d_4 = nn.Conv3d(in_channels=64, out_channels=16, kernel_size=3, stride=2, padding=1)
            self.conv3d_5 = nn.Conv3d(in_channels=16, out_channels=4, kernel_size=3, stride=2, padding=1)
            self.conv3d_6 = nn.Conv3d(in_channels=4, out_channels=1, kernel_size=3, stride=2, padding=1)
            self.bn4 = nn.BatchNorm3d(16)
            self.bn5 = nn.BatchNorm3d(4)
            self.bn6 = nn.BatchNorm3d(1)

            self.linear = nn.Linear(4*4*4, 5)

        def forward(self, x):
            x1, x2 = x.permute(0, 2, 3, 1), x.permute(0, 3, 2, 1)
        
            x1 = self.prelux_x1_1(self.bn_x1_1(self.conv2d_x1_1(x1))).permute(0, 2, 1, 3)
            x2 = self.prelux_x2_1(self.bn_x2_1(self.conv2d_x2_1(x2))).permute(0, 2, 1, 3)
            x1 = self.prelux_x1_2(self.bn_x1_2(self.conv2d_x1_2(x1))).permute(0, 3, 2, 1)
            x2 = self.prelux_x2_2(self.bn_x2_2(self.conv2d_x2_2(x2))).permute(0, 3, 2, 1)
            x1 = self.prelux_x1_3(self.bn_x1_3(self.conv2d_x1_3(x1))).squeeze(1)
            x2 = self.prelux_x2_3(self.bn_x2_3(self.conv2d_x2_3(x2))).squeeze(1)

            x = torch.stack([x1, x2], dim=1)
            x = self.prelux0(self.bn0(self.conv2d(x))).unsqueeze(0)

            x = self.prelux1(self.bn1(self.conv3d_1(x)))
            x = self.prelux1(self.bn2(self.conv3d_2(x)))
            x = self.prelux1(self.bn3(self.conv3d_3(x)))

            x = F.relu(self.bn4(self.conv3d_4(x)))
            x = F.relu(self.bn5(self.conv3d_5(x)))
            x = F.relu(self.bn6(self.conv3d_6(x)))
