...
def train_epoch(model, loader, optimizer, scaler, epoch, args):
    ...
    for idx, batch_data in enumerate(loader):
        ...
        with autocast(enabled=args.amp):
            logits = model(data.real.to(torch.float32).squeeze(1))
...
def main_worker(gpu, args):
    ...
    class MyModel(nn.Module):
        def __init__(self):
            ...
            self.conv1 = nn.Conv2d(3, 6, kernel_size=1)
            self.bn1 = nn.BatchNorm2d(6, affine=True)
            self.prelu1 = nn.PReLU()

            self.conv2 = nn.Conv2d(6, 9, kernel_size=1)
            self.bn2 = nn.BatchNorm2d(9, affine=True)
            self.prelu2 = nn.PReLU()

            self.conv3 = nn.Conv2d(9, 18, kernel_size=1)
            self.bn3 = nn.BatchNorm2d(18, affine=True)
            self.prelu3 = nn.PReLU()

            self.conv4 = nn.Conv2d(18, 9, kernel_size=3, padding=1, stride=2)
            self.bn4 = nn.BatchNorm2d(9, affine=True)
            self.prelu4 = nn.PReLU()

            self.conv5 = nn.Conv2d(9, 6, kernel_size=3, padding=1, stride=2)
            self.bn5 = nn.BatchNorm2d(6, affine=True)
            self.prelu5 = nn.PReLU()

            self.conv6 = nn.Conv2d(6, 3, kernel_size=3, padding=1, stride=2)
            self.bn6 = nn.BatchNorm2d(3, affine=True)
            self.prelu6 = nn.PReLU()

            self.conv0 = nn.Conv2d(3, 1, kernel_size=3, padding=1, stride=2)
            self.bn0 = nn.BatchNorm2d(1, affine=True)
            self.prelu0 = nn.PReLU()

            self.conv00 = nn.Conv2d(1, 1, kernel_size=3, padding=1, stride=2)
            self.bn00 = nn.BatchNorm2d(1, affine=True)
            self.prelu00 = nn.PReLU()
        ...
        def forward(self, x):
            ...
            x = self.prelu00(self.bn00(self.conv00(x)))
...
def parse_args():
    ...
    parser.add_argument(
        "--data_root", default="/data/breast-cancer/PANDA/train_images_FFT3/", help="path to root folder of images"
    )
