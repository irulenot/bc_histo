...
def main_worker(gpu, args):
    ...
    class MyModel(nn.Module):
        def __init__(self):
            ...
            self.conv5 = nn.Conv2d(64, 32, kernel_size=3, padding=1, stride=2)
            self.bn5 = nn.BatchNorm2d(32, affine=True)
            self.prelu5 = nn.PReLU()

            self.conv6 = nn.Conv2d(32, 16, kernel_size=3, padding=1, stride=2)
            self.bn6 = nn.BatchNorm2d(16, affine=True)
            self.prelu6 = nn.PReLU()

            self.conv0 = nn.Conv2d(16, 8, kernel_size=3, padding=1, stride=2)
            self.bn0 = nn.BatchNorm2d(8, affine=True)
            self.prelu0 = nn.PReLU()

            self.conv00 = nn.Conv2d(8, 1, kernel_size=3, padding=1, stride=2)
