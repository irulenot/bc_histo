...
def train_epoch(model, loader, optimizer, scaler, epoch, args):
    ...
    for idx, batch_data in enumerate(loader):
        ...
        quadrant1 = data[:, :, :1024, :1024]
        quadrant2 = data[:, :, :1024, 1024:]
        quadrant3 = data[:, :, 1024:, :1024]
        quadrant4 = data[:, :, :1024, 1024:]
        # Stack the quadrants along a new dimension to get the desired shape
        data = torch.stack([quadrant1, quadrant2, quadrant3, quadrant4], dim=1)
    ...
    with torch.no_grad():
        for idx, batch_data in enumerate(loader):
            ...
            with autocast(enabled=args.amp):
                ...
                # Stack the quadrants along a new dimension to get the desired shape
                data = torch.stack([quadrant1, quadrant2, quadrant3, quadrant4], dim=1)
                logits = model(data.squeeze()[0].unsqueeze(0).unsqueeze(0))
...
import imageio
def load_image(x):
    image = imread(x["image"]).real
    image = (image - image.min()) / (image.max() - image.min())
    return {"image":image.astype(np.float32), "label": x['label']}
...
def main_worker(gpu, args):
    ...
    model = milmodel.MILModel(num_classes=args.num_classes, pretrained=False, mil_mode=args.mil_mode)
...
def parse_args():
    ...
    parser.add_argument(
        "--data_root", default="/data/breast-cancer/PANDA/train_images_FFT2/", help="path to root folder of images"
    )
