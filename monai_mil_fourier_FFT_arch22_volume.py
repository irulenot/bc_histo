...
def main_worker(gpu, args):
    ...
    class MyModel(nn.Module):
        def __init__(self):
            ...
            self.conv31 = nn.Conv2d(64, 32, kernel_size=3, stride=1, padding=1)
            self.bn31 = nn.BatchNorm2d(32, affine=True)
            self.prelu31 = nn.ReLU()
            self.conv32 = nn.Conv2d(64, 32, kernel_size=3, stride=1, padding=1)
            self.bn32 = nn.BatchNorm2d(32, affine=True)
            self.prelu32 = nn.ReLU()
            self.conv33 = nn.Conv2d(64, 32, kernel_size=3, stride=1, padding=1)
            self.bn33 = nn.BatchNorm2d(32, affine=True)
            self.prelu33 = nn.ReLU()
            self.conv34 = nn.Conv2d(32, 16, kernel_size=3, stride=1, padding=1)
            self.bn34 = nn.BatchNorm2d(16, affine=True)
            self.prelu34 = nn.ReLU()
            self.conv35 = nn.Conv2d(32, 8, kernel_size=3, stride=1, padding=1)
            self.bn35 = nn.BatchNorm2d(8, affine=True)
            self.prelu35 = nn.ReLU()
            self.conv36 = nn.Conv2d(32, 8, kernel_size=3, stride=1, padding=1)
            self.bn36 = nn.BatchNorm2d(8, affine=True)
            self.prelu36 = nn.ReLU()

            self.linear41 = nn.Linear(1024, 512)
            self.bn41 = nn.LayerNorm(512, elementwise_affine=True)
            self.prelu41 = nn.ReLU()
        ...
        def forward(self, x):
            ...
            x = x.permute(0, 3, 1, 2)
            x = self.prelu31(self.bn31(self.conv31(x)))
            x = x.permute(0, 3, 1, 2)
            x = self.prelu32(self.bn32(self.conv32(x)))
            x = x.permute(0, 3, 1, 2)            
            x = self.prelu33(self.bn33(self.conv33(x)))
            x = x.permute(0, 3, 1, 2)
            x = self.prelu34(self.bn34(self.conv34(x)))
            x = x.permute(0, 3, 1, 2)
            x = self.prelu35(self.bn35(self.conv35(x)))
            x = x.permute(0, 3, 1, 2)            
            x = self.prelu36(self.bn36(self.conv36(x)))

            x = x.permute(0, 3, 1, 2)
            x = x.reshape(x.shape[0], -1)
            x = self.prelu41(self.bn41(self.linear41(x)))
