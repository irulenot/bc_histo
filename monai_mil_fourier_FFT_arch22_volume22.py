...
def main_worker(gpu, args):
    ...
    class MyModel(nn.Module):
        def __init__(self):
            ...
            self.conv2 = nn.Conv2d(8, 32, kernel_size=1, stride=1)
            ...
            self.conv4 = nn.Conv2d(64, 128, kernel_size=1, stride=1)
            ...
            # Rows
            self.conv012 = nn.Conv2d(512, 256, kernel_size=3, stride=1, padding=1)
            self.bn012 = nn.BatchNorm2d(256, affine=True)
            self.prelu012 = nn.PReLU()
            self.conv013 = nn.Conv2d(256, 128, kernel_size=3, stride=1, padding=1)
            self.bn013 = nn.BatchNorm2d(128, affine=True)
            self.prelu013 = nn.PReLU()
            self.conv014 = nn.Conv2d(128, 64, kernel_size=3, stride=1, padding=1)
            self.bn014 = nn.BatchNorm2d(64, affine=True)
            self.prelu014 = nn.PReLU()
            self.conv015 = nn.Conv2d(64, 32, kernel_size=3, stride=1, padding=1)
            self.bn015 = nn.BatchNorm2d(32, affine=True)
            self.prelu015 = nn.PReLU()
            self.conv016 = nn.Conv2d(32, 16, kernel_size=3, stride=1, padding=1)
            self.bn016 = nn.BatchNorm2d(16, affine=True)
            self.prelu016 = nn.PReLU()
            self.conv017 = nn.Conv2d(16, 8, kernel_size=3, stride=1, padding=1)
            self.bn017 = nn.BatchNorm2d(8, affine=True)
            self.prelu017 = nn.PReLU()           

            # Rows
            self.conv11 = nn.Conv2d(1024, 512, kernel_size=3, stride=1, padding=1)
            self.bn11 = nn.BatchNorm2d(512, affine=True)
            self.prelu11 = nn.PReLU()
            self.conv12 = nn.Conv2d(512, 256, kernel_size=3, stride=1, padding=1)
            self.bn12 = nn.BatchNorm2d(256, affine=True)
            self.prelu12 = nn.PReLU()
            self.conv13 = nn.Conv2d(256, 128, kernel_size=3, stride=1, padding=1)
            self.bn13 = nn.BatchNorm2d(128, affine=True)
            self.prelu13 = nn.PReLU()
            self.conv14 = nn.Conv2d(128, 64, kernel_size=3, stride=1, padding=1)
            self.bn14 = nn.BatchNorm2d(64, affine=True)
            self.prelu14 = nn.PReLU()
            self.conv15 = nn.Conv2d(64, 32, kernel_size=3, stride=1, padding=1)
            self.bn15 = nn.BatchNorm2d(32, affine=True)
            self.prelu15 = nn.PReLU()
            self.conv16 = nn.Conv2d(32, 16, kernel_size=3, stride=1, padding=1)
            self.bn16 = nn.BatchNorm2d(16, affine=True)
            self.prelu16 = nn.PReLU()
            self.conv17 = nn.Conv2d(16, 8, kernel_size=3, stride=1, padding=1)
            self.bn17 = nn.BatchNorm2d(8, affine=True)
            self.prelu17 = nn.PReLU()   

            # Columns
            self.conv21 = nn.Conv2d(1024, 512, kernel_size=3, stride=1, padding=1)
            self.bn21 = nn.BatchNorm2d(512, affine=True)
            self.prelu21 = nn.PReLU()
            self.conv22 = nn.Conv2d(512, 256, kernel_size=3, stride=1, padding=1)
            self.bn22 = nn.BatchNorm2d(256, affine=True)
            self.prelu22 = nn.PReLU()
            self.conv23 = nn.Conv2d(256, 128, kernel_size=3, stride=1, padding=1)
            self.bn23 = nn.BatchNorm2d(128, affine=True)
            self.prelu23 = nn.PReLU()
            self.conv24 = nn.Conv2d(128, 64, kernel_size=3, stride=1, padding=1)
            self.bn24 = nn.BatchNorm2d(64, affine=True)
            self.prelu24 = nn.PReLU()
            self.conv25 = nn.Conv2d(64, 32, kernel_size=3, stride=1, padding=1)
            self.bn25 = nn.BatchNorm2d(32, affine=True)
            self.prelu25 = nn.PReLU()
            self.conv26 = nn.Conv2d(32, 16, kernel_size=3, stride=1, padding=1)
            self.bn26 = nn.BatchNorm2d(16, affine=True)
            self.prelu26 = nn.PReLU()
            self.conv27 = nn.Conv2d(16, 8, kernel_size=3, stride=1, padding=1)
            self.bn27 = nn.BatchNorm2d(8, affine=True)
            self.prelu27 = nn.PReLU()   

            self.linear42 = nn.Linear(512, 256)
            self.bn42 = nn.LayerNorm(256, elementwise_affine=True)
            self.prelu42 = nn.ReLU()
            self.linear43 = nn.Linear(256, 128)
            self.bn43 = nn.LayerNorm(128, elementwise_affine=True)
            self.prelu43 = nn.ReLU()
            self.linear44 = nn.Linear(128, 64)
            self.bn44 = nn.LayerNorm(64, elementwise_affine=True)
            self.prelu44 = nn.ReLU()
            self.linear45 = nn.Linear(64, 32)
            self.bn45 = nn.LayerNorm(32, elementwise_affine=True)
            self.prelu45 = nn.ReLU()
            self.linear46= nn.Linear(32, 5)
        ...
        def forward(self, x):
            ...
            x = x.permute(0, 3, 1, 2)
            x = self.prelu012(self.bn012(self.conv012(x)))

            x = x.permute(0, 3, 1, 2)
            x = self.prelu13(self.bn13(self.conv13(x)))
            x = x.permute(0, 3, 1, 2)
            x = self.prelu23(self.bn23(self.conv23(x)))
            x = x.permute(0, 3, 1, 2)
            x = self.prelu013(self.bn013(self.conv013(x)))

            x = x.permute(0, 3, 1, 2)
            x = self.prelu14(self.bn14(self.conv14(x)))
            x = x.permute(0, 3, 1, 2)
            x = self.prelu24(self.bn24(self.conv24(x)))
            x = x.permute(0, 3, 1, 2)
            x = self.prelu014(self.bn014(self.conv014(x)))

            x = x.permute(0, 3, 1, 2)
            x = self.prelu15(self.bn15(self.conv15(x)))
            x = x.permute(0, 3, 1, 2)
            x = self.prelu25(self.bn25(self.conv25(x)))
            x = x.permute(0, 3, 1, 2)
            x = self.prelu015(self.bn015(self.conv015(x)))

            x = x.permute(0, 3, 1, 2)
            x = self.prelu16(self.bn16(self.conv16(x)))
            x = x.permute(0, 3, 1, 2)
            x = self.prelu26(self.bn26(self.conv26(x)))
            x = x.permute(0, 3, 1, 2)
            x = self.prelu016(self.bn016(self.conv016(x)))

            x = x.permute(0, 3, 1, 2)
            x = self.prelu17(self.bn17(self.conv17(x)))
            x = x.permute(0, 3, 1, 2)
            x = self.prelu27(self.bn27(self.conv27(x)))
            x = x.permute(0, 3, 1, 2)
            x = self.prelu017(self.bn017(self.conv017(x)))

            x = x.view(x.shape[0], -1)
            x = self.prelu42(self.bn42(self.linear42(x)))
            x = self.prelu43(self.bn43(self.linear43(x)))
            x = self.prelu44(self.bn44(self.linear44(x)))
            x = self.prelu45(self.bn45(self.linear45(x)))
            x = self.linear46(x)
            return x
    model = MyModel()
