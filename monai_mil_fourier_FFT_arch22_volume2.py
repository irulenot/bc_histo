...
def main_worker(gpu, args):
    ...
    class MyModel(nn.Module):
        def __init__(self):
            ...
            self.bn11 = nn.BatchNorm2d(64, affine=True)
            self.prelu11 = nn.PReLU()
            self.conv12 = nn.Conv2d(64, 64, kernel_size=1, stride=1)
            self.bn12 = nn.BatchNorm2d(64, affine=True)
            self.prelu12 = nn.PReLU()

            # Rows2
            self.conv011 = nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1)
            self.bn011 = nn.BatchNorm2d(64, affine=True)
            self.prelu011 = nn.PReLU()
            self.conv012 = nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1)
            self.bn012 = nn.BatchNorm2d(64, affine=True)
            self.prelu012 = nn.PReLU()

            # Columns
            self.conv14 = nn.Conv2d(64, 64, kernel_size=1, stride=1)
            self.bn14 = nn.BatchNorm2d(64, affine=True)
            self.prelu14 = nn.PReLU()
            self.conv15 = nn.Conv2d(64, 64, kernel_size=1, stride=1)
            self.bn15 = nn.BatchNorm2d(64, affine=True)
            self.prelu15 = nn.PReLU()

            # Columns2
            self.conv014 = nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1)
            self.bn014 = nn.BatchNorm2d(64, affine=True)
            self.prelu014 = nn.PReLU()
            self.conv015 = nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1)
            self.bn015 = nn.BatchNorm2d(64, affine=True)
            self.prelu015 = nn.PReLU()

            # Create volume from rows and columns
            self.conv21 = nn.Conv3d(4, 8, kernel_size=3, padding=1, stride=2)
            self.bn21 = nn.BatchNorm3d(8, affine=True)
            self.prelu21 = nn.PReLU()
            self.conv22 = nn.Conv3d(8, 16, kernel_size=3, padding=1, stride=2)
            self.bn22 = nn.BatchNorm3d(16, affine=True)
            self.prelu22 = nn.PReLU()
            self.conv23 = nn.Conv3d(16, 8, kernel_size=3, padding=1, stride=2)
            self.bn23 = nn.BatchNorm3d(8, affine=True)
            self.prelu23 = nn.PReLU()
            self.conv24 = nn.Conv3d(8, 4, kernel_size=3, padding=1, stride=2)
            self.bn24 = nn.BatchNorm3d(4, affine=True)
            self.prelu24 = nn.PReLU()
            self.conv25 = nn.Conv3d(4, 1, kernel_size=3, padding=1, stride=2)
            self.bn25 = nn.BatchNorm3d(1, affine=True)
            self.prelu25 = nn.PReLU()

            self.linear41 = nn.Linear(2048, 1024)
            self.bn41 = nn.LayerNorm(1024, elementwise_affine=True)
            self.prelu41 = nn.PReLU()
            self.linear42 = nn.Linear(1024, 100)
            self.bn42 = nn.LayerNorm(100, elementwise_affine=True)
            self.prelu42 = nn.PReLU()
            self.linear43 = nn.Linear(100, 10)
            self.bn43 = nn.LayerNorm(10, elementwise_affine=True)
            self.prelu43 = nn.PReLU()
            self.linear44 = nn.Linear(10, 5)
        ...
        def forward(self, x):
            ...
            x1, x2 = x.clone(), x.clone().permute(0, 1, 3, 2)
            x1 = self.prelu11(self.bn11(self.conv11(x1)))
            x1 = self.prelu12(self.bn12(self.conv12(x1)))
            x2 = self.prelu14(self.bn14(self.conv14(x2)))
            x2 = self.prelu15(self.bn15(self.conv15(x2)))
            x3, x4 = x.clone(), x.clone().permute(0, 1, 3, 2)
            x3 = self.prelu011(self.bn11(self.conv011(x3)))
            x3 = self.prelu012(self.bn12(self.conv012(x3)))
            x4 = self.prelu014(self.bn14(self.conv014(x4)))
            x4 = self.prelu015(self.bn15(self.conv015(x4)))

            x0 = torch.stack([x1, x2.permute(0, 1, 3, 2), x3, x4.permute(0, 1, 3, 2)], dim=1)
            ...
            x0 = self.prelu25(self.bn25(self.conv25(x0)))

            x0 = x0.view(x0.shape[0], -1)
            x0 = self.prelu41(self.bn41(self.linear41(x0)))
            x0 = self.prelu42(self.bn42(self.linear42(x0)))
            x0 = self.prelu43(self.bn43(self.linear43(x0)))
            x0 = self.linear44(x0)
            return x0
    model = MyModel()
...
def parse_args():
    ...
    parser.add_argument("--optim_lr", default=3e-4, type=float, help="initial learning rate")
