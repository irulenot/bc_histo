...
def fft_transform(batch_data, device):
    ...
    image = torch.mean(image.to(torch.float32), dim=0, keepdim=True)
...
def parse_args():
    ...
    parser.add_argument(
        "--data_root", default="/data/breast-cancer/PANDA/train_images_FFT_WSI_grayscaled4/", help="path to root folder of images"
    )
...
if __name__ == "__main__":
    ...
    if args.dataset_json is None:
        ...
        if args.quick == True:
            dst = "datalists/datalist_panda_fft_quick4.json"
        else:
            dst = "datalists/datalist_panda_fft4.json"
