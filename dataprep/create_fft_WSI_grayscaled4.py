...
def process_tiff(tiff_file):
    ...
    horizontal = fft_img[start_h:start_h+crop_size, :]
    vertical = fft_img[:, start_w:start_w+crop_size]
    
    np.savez_compressed(f'{output_file}', array1=horizontal.numpy(), array2=vertical.numpy())
    return output_file
