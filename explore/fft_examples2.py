...
def train_epoch(model, loader, optimizer, scaler, epoch, args):
    ...
    for idx, batch_data in enumerate(loader):
        ...
        data_cpu = data.cpu()
        data_np = data_cpu.squeeze().numpy()
        data_np = data_np[0]
        ...
        plt.colorbar()
        plt.imsave('outputs/initial_image.png', data_np, cmap='gray')

        # Compute the Fourier transform
        fft_data = np.fft.fft2(data_np)
        ...
        plt.colorbar()
        plt.imsave('outputs/magnitude_spectrum.png', np.log(magnitude_spectrum + 1), cmap='gray')
        # Perform inverse Fourier transform to reconstruct the image
        reconstructed_image = np.fft.ifft2(np.fft.ifftshift(fft_data_shifted)).real
        # Save the reconstructed image
        plt.imshow(reconstructed_image, cmap='gray')
        plt.title('Reconstructed Image')
        plt.colorbar()
        plt.imsave('outputs/reconstructed_image.png', reconstructed_image, cmap='gray')

        # Define a simple low-pass filter mask
        rows, cols = data_np.shape
        crow, ccol = rows // 2, cols // 2
        mask = np.zeros((rows, cols), np.uint8)
        radius = 1000  # Adjust this value to change the cutoff frequency
        mask[crow - radius:crow + radius, ccol - radius:ccol + radius] = 1
        # Apply the low-pass filter
        fft_data_shifted_filtered = fft_data_shifted * mask
        # Save the filtered FFT image
        plt.imshow(np.log(np.abs(fft_data_shifted_filtered) + 1), cmap='gray')
        plt.title('Filtered FFT Image')
        plt.colorbar()
        plt.imsave('outputs/filtered_fft_image_100.png', np.log(np.abs(fft_data_shifted_filtered) + 1), cmap='gray')
        # Perform inverse Fourier transform to reconstruct the image
        reconstructed_image = np.fft.ifft2(np.fft.ifftshift(fft_data_shifted_filtered)).real
        # Save the reconstructed image
        plt.imshow(reconstructed_image, cmap='gray')
        plt.title('Reconstructed Image')
        plt.colorbar()
        plt.imsave('outputs/reconstructed_image_100.png', reconstructed_image, cmap='gray')
        ##########################
        finished = True
