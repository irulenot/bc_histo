...
def process_tiff(tiff_file):
    ...
    fft_img1 = torch.fft.fftshift(fft_img, dim=(-2, -1))
    ...
    for crop_size in crop_sizes:
        ...
        mask[:, start_w:start_w+crop_size] = 1

        cropped_fft_img = fft_img1 * mask
        ...
        save_path = os.path.join(output_dir, f'cross_shift_image_{crop_size}.jpg')
    ...
    for crop_size in crop_sizes:
        ...
        # Perform inverse FFT and convert to numpy array
        cropped_fft_img = torch.fft.ifft2(cropped_fft_img, dim=(-2, -1))
        cropped_fft_img = cropped_fft_img.squeeze().cpu().numpy()
        
        # Convert to PIL image and save
        pil_image = Image.fromarray(cropped_fft_img.astype('uint8'))
        save_path = os.path.join(output_dir, f'cross_image_{crop_size}.jpg')
        pil_image.save(save_path)
    ...
    for crop_size in crop_sizes:
        ...
        cropped_fft_img = cropped_fft_img.squeeze().cpu().numpy()
        pil_image = Image.fromarray(cropped_fft_img.astype('uint8'))
        save_path = os.path.join(output_dir, f'reconstructed_image_noshift_{crop_size}.jpg')
        pil_image.save(save_path)
    ...
    for crop_size in crop_sizes:
        ...
        cropped_fft_img = cropped_fft_img.squeeze().cpu().numpy()
        pil_image = Image.fromarray(cropped_fft_img.astype('uint8'))
        save_path = os.path.join(output_dir, f'quart_{crop_size}.jpg')
        pil_image.save(save_path)
