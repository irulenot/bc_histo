...
def process_tiff(tiff_file):
    ...
    rev_img = torch.fft.ifft2(fft_img, dim=(-2, -1))
    ...
    for crop_size in crop_sizes:
        ...
        pil_image = Image.fromarray(cropped_fft_img.astype('uint8'))
        save_path = os.path.join(output_dir, f'bottom_left_reconstructed_image_{crop_size}.jpg')
        pil_image.save(save_path)
...
result = process_tiff(tiff_files[1])
