...
def fft_transform(batch_data, device):
    image = torch.load(batch_data['image'][0])
    # image = image.real
...
def main_worker(gpu, args):
    ...
    for i , path in enumerate(training_list):
        training_list[i]['image'] = training_list[i]['image'][:-5] + '.pth'
    for i , path in enumerate(validation_list):
        validation_list[i]['image'] = validation_list[i]['image'][:-5] + '.pth'
...
def parse_args():
    ...
    parser.add_argument(
        "--data_root", default="/data/breast-cancer/PANDA/train_images_FFT500_WSI_grayscaled/", help="path to root folder of images"
    )
